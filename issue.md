...
### 1. PDF页码同步问题（2025年5月15日）

1. **问题描述**：
   - PDF文档滚动时，页码选择器不能实时同步当前页码
   - 页码选择器显示的页码与实际查看的页面不一致
   - 快速滚动时，页码显示混乱或停留在某一页不变
   - 页码同步不稳定，有时需要额外滚动才能更新

2. **问题分析**：
   - 当前页码检测逻辑基于页面中心点与容器中心点的距离
   - 相邻页面的距离可能非常接近，导致判断不准确
   - 滚动事件触发频率可能不足以捕捉所有页面变化
   - 缺乏对滚动方向的考虑，导致页码在相邻页面间来回跳动

3. **尝试的解决方案**：
   - 使用IntersectionObserver API监测页面可见性（效果不佳）
   - 优化页面可见度计算逻辑（部分改善但不稳定）
   - 考虑滚动方向和页面可见比例（复杂度增加但效果有限）
   - 添加滚动防抖和节流处理（减少了跳动但响应不及时）

4. **后续计划**：
   - 研究PDF.js原生页码同步机制
   - 考虑使用滚动位置与页面高度的比例计算当前页码
   - 探索更简单可靠的页码检测方法
   - 可能需要重新设计页面滚动和页码同步逻辑
